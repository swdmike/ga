...
public class ProjectUtil
{
    ...
    {
        Polygon polygon = GeometryUtil.createPolygonFromWKT(wkt);
        if (polygon == null)
        {
            return "";
        }
        Polygon pg = CovertPolygonJWDToXY(polygon, ellipse, centralM, dai);
        wkt = GeometryUtil.createWKTFromPolygon(polygon);
        return wkt;
    }
     ...
     * 功能描述:经纬度转平面坐标
     */
    private static double[] BLToXY(double longitude, double latitude, int ellipse, double centralM, int dai)
    {
    ...
    /**
     * 平面坐标转经纬度
     * 调用时反一下坐标: XYToBL(y, x - dai * 1000000, eclipse, centralM);
     *
     * @param: ellipse 2000或者1980
     * @return: [longitude, latitude]
     */
    private static double[] XYToBL(double x, double y, int ellipse, double centralM)
    {
        double a = 6378137.0;
        double b = 6356752.314140356;
        if (ellipse == 1980)
